...
class Graph extends Component {
  ...
  componentDidMount() {
    axios.get('http://43.204.211.170:8089/bmi')
      .then(response => {
  ...
  handleSubmitSaveData = async (event) => {
    ...
    try {
      ...
      const response = await axios.post('http://43.204.211.170:8089/postbmi', {
  ...
  async handleRemoveClick(index, date) {
    try {
      ...
      const response = await axios.delete(`http://43.204.211.170:8089/delbmi?date=${date}`);
